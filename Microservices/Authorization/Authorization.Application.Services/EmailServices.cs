...
namespace Authorization.Application.Services
{
    ...
    {
        public Task SendConfirmEmailAsync(Account userId, string subject, string token, CancellationToken cancellationToken);
    }
    ...
    public class EmailServices : IEmailServices
    {
        ...
        public async Task SendConfirmEmailAsync(Account user, string subject, string token, CancellationToken cancellationToken)
        {
            var encodedToken = HttpUtility.UrlEncode(token);
            var callbackUrl = $"https://localhost:44336/api/User/confirmation/{user.Id}?token={encodedToken}";
            var content =string.Format(Messages.HtmlMessage,string.Join(" ",user.LastName,user.FirstName,user.MiddleName), callbackUrl);
