...
namespace Authorization.Data.Repository
{
    ...
    {
        public Task<AppointmentScheduleForDoctorDTO[]> GetAppointmentScheduleForDoctorByCurrentDayAsync(DateTime date,long doctorId,
    ...
    internal class AppointmentRepository : RepositoryBase<AppointmentEntity>, IAppointmentRepository
    {
        ...
        public async Task<AppointmentScheduleForDoctorDTO[]> GetAppointmentScheduleForDoctorByCurrentDayAsync(DateTime date,long doctorId,
            CancellationToken cancellationToken = default)
        {
            return await DbContext.Appointments.AsNoTracking().Where(x => x.DateTime.Date == date.Date && x.DoctorId==doctorId)
        ...
        public async Task<AppointmentScheduleForReceptionistDTO[]> GetAppointmentScheduleForReceptionistByCurrentDayAsync(DateTime date,long officeId,
            CancellationToken cancellationToken = default)
        {
            return await DbContext.Appointments.AsNoTracking().Where(x => x.DateTime.Date == date.Date && x.OfficeId==officeId)
