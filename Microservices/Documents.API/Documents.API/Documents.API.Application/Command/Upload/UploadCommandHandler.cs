...
using Documentation = Authorization.Data_Domain.Models.Documentation;
...
namespace Documents.API.Application.Command.Upload
{
    internal class UploadCommandHandler : IRequestHandler<UploadCommand, long>
    {
        ...
        public UploadCommandHandler(IBlobServices blobService,IOptions<BlobStorageSettings> blobStorageSettings,IDocumentsRepository documentsRepository)
        {
        ...
        public async Task<long> Handle(UploadCommand request, CancellationToken cancellationToken)
        {
            ...
            string pathTemplate = GetTemplatePath(request.Subject);
